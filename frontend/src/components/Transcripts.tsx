...
function parseReportBlocks(markdown: string): ReportBlock[] {
  const lines = normalizeReportMarkdown(markdown).split('\n');
  ...
  while (index < lines.length) {
    ...
    if (tableHeader?.includes('|') && tableDivider && isMarkdownTableSeparator(tableDivider.trim())) {
      const headers = parseMarkdownTableRow(tableHeader);
      const rows: string[][] = [];
      index += 2;
    ...
    while (index < lines.length) {
      ...
      if (
        ...
        currentLine.startsWith('- ') ||
        (currentLine.includes('|') && nextLine && isMarkdownTableSeparator(nextLine))
      ) {
        break;
      }
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between mb-8">
          ...
          <div className="flex flex-wrap items-center gap-3 xl:justify-end">
            ...
            <button
              ...
              className={`inline-flex min-h-11 items-center justify-center gap-2 px-4 py-2 text-sm font-medium rounded-md transition-colors whitespace-nowrap flex-1 lg:flex-none ${dark ? 'bg-gray-800 hover:bg-gray-700 text-gray-200' : 'bg-white border border-gray-200 hover:bg-gray-50 text-gray-700'
                }`}
            >
            ...
            {conversation.report_text && (
              <button
                ...
                className={`inline-flex min-h-11 items-center justify-center gap-2 px-4 py-2 text-sm font-medium rounded-md transition-colors whitespace-nowrap flex-1 lg:flex-none ${dark ? 'bg-sky-600/20 text-sky-400 hover:bg-sky-600/30' : 'bg-sky-50 text-sky-700 hover:bg-sky-100'
                  }`}
              >
                <Download className="w-4 h-4" />
                Report
...
function ReportTab({ conversation, isDarkMode }: { conversation: ConversationFull; isDarkMode: boolean }) {
  const dark = isDarkMode;
  const reportBlocks = parseReportBlocks(conversation.report_text ?? '');
          ...
          <div className="space-y-6">
            {reportBlocks.map((block, index) => {
              if (block.type === 'divider') {
                return <div key={`divider-${index}`} className={`${dark ? 'border-gray-800' : 'border-gray-200'} border-t`} />;
              }

              if (block.type === 'heading') {
                if (block.level === 1) {
                  return (
                    <h1 key={`heading-${index}`} className={`text-3xl font-semibold tracking-tight ${dark ? 'text-gray-100' : 'text-gray-900'}`}>
                      <MarkdownInline content={block.content} />
                    </h1>
                  );
                }
                ...
                if (block.level === 2) {
                  return (
                    <h2 key={`heading-${index}`} className={`text-xl font-semibold tracking-tight ${dark ? 'text-gray-100' : 'text-gray-900'}`}>
                      <MarkdownInline content={block.content} />
                    </h2>
                  );
                }
                ...
                return (
                  <h3 key={`heading-${index}`} className={`${dark ? 'text-sky-300' : 'text-sky-700'} text-lg font-semibold`}>
                    <MarkdownInline content={block.content} />
                  </h3>
                );
              }
              ...
              if (block.type === 'paragraph') {
                return (
                  <div key={`paragraph-${index}`} className={`${dark ? 'text-gray-300' : 'text-gray-700'} text-[15px] leading-7`}>
                    <ReactMarkdown
                      components={{
                        p: ({ children }) => <p className="mb-0">{children}</p>,
                        strong: ({ children }) => <strong className={`${dark ? 'text-gray-100' : 'text-gray-900'} font-semibold`}>{children}</strong>,
                      }}
                    >
                      {block.content}
                    </ReactMarkdown>
                ...
                return (
                  <ul key={`list-${index}`} className="space-y-3">
                    {block.items.map((item, itemIndex) => (
                      <li key={`list-item-${itemIndex}`} className="flex items-start gap-3">
                        <span className={`mt-2 h-2 w-2 rounded-full ${dark ? 'bg-sky-400' : 'bg-sky-600'}`} />
                        <div className={`${dark ? 'text-gray-300' : 'text-gray-700'} text-[15px] leading-7`}>
              ...
              return (
                <div key={`table-${index}`} className={`overflow-hidden rounded-2xl border ${dark ? 'border-gray-800 bg-[#101010]' : 'border-gray-200 bg-gray-50/70'}`}>
                  <div className="overflow-x-auto">
                    <table className="min-w-full border-collapse text-sm">
                      <thead>
                        <tr className={dark ? 'bg-gray-900/80' : 'bg-white/90'}>
                          {block.headers.map((header) => (
                            <th
                              key={header}
                              className={`px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.18em] ${dark ? 'text-gray-400' : 'text-gray-500'}`}
                            >
                      ...
                      <tbody>
                        {block.rows.map((row, rowIndex) => (
                          <tr key={`row-${rowIndex}`} className={`align-top ${rowIndex !== block.rows.length - 1 ? (dark ? 'border-b border-gray-800' : 'border-b border-gray-200') : ''}`}>
                            {row.map((cell, cellIndex) => (
                              <td
                                key={`cell-${rowIndex}-${cellIndex}`}
                                className={`px-4 py-3 leading-6 ${dark ? 'text-gray-300' : 'text-gray-700'} ${cellIndex === 0 ? 'font-medium' : ''}`}
                              >
